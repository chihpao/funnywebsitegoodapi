...
const InteractiveComponent = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setIsExiting(true); // 先觸發淡出動畫
      setTimeout(() => {
        setCurrentNameIndex((prevIndex) => (prevIndex + 1) % names.length);
        setIsExiting(false); // 換字後淡入動畫
        setIsFadingIn(true); // 觸發從下面往上的淡入動畫
        setTimeout(() => {
          setIsFadingIn(false); // 淡入動畫結束
        }, 500); // 與 CSS 的 `animation` 時間保持一致
      }, 500); // 與 CSS 的 `transition` 時間保持一致
    }, 2850); // 每 2.85 秒切換
        ...
        <div className="container mx-auto text-left">
          <div className="text-lg">
            ...
            <div className="highlight-blackbg mt-4">
              <div
                className={`highlight-content ${isExiting ? 'exiting' : 'active'} ${isFadingIn ? 'fade-in-up' : ''} text-white`}
              >
          ...
          <div className="text-lg mt-4">
            <div className="highlight-whitebg mt-4">
              <div
                className={`highlight-content ${isExiting ? 'exiting' : 'active'} ${isFadingIn ? 'fade-in-up' : ''}`}
              >
                2022
              </div>
            </div>
